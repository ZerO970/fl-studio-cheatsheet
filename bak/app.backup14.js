...
(function () {
  ...
  window.openCategoryScreen = function(cat) {
    ...
    if (!onFeature) {
      navPush('На главную', () => { navClearHistory(); showWelcome(); });
    }
  ...
  const backBtn = (overrideLabel) => {
    ...
    const label = overrideLabel || (prev ? prev.label : 'На главную');
    return `
  ...
  window.openChainDetail = function(id) {
    ...
    if (!chain) return;
    navPush('Цепочки', () => renderChainsScreen());
  ...
  window.selectPluginFromChain = function(id, chainId) {
    if (chainId) navPush('Цепочка', () => openChainDetail(chainId));
    selectPlugin(id);
  };
  ...
  window.selectPlugin = function (id) {
    ...
    if (fs && !fs.classList.contains('hidden')) {
      navPush(prevId ? currentPlugin.name : 'Назад', () => {
  ...
  function renderParams(p) {
    ...
    if (!params || !params.length) {
      el.innerHTML = `<div class="params-empty">
        ...
        <div>Таблица параметров для этого плагина в разработке</div>
      </div>`;
      return;
    }

    const hasScreenshot  = !!p.screenshot;
    const hasMarkers     = params.some(pr => pr.x != null);
    ...
    const screenshotHtml = hasScreenshot ? `
      <div class="params-shot-wrap">
        ...
        ${hasMarkers ? params.map(pr => pr.x != null ? `
          <div class="param-marker" data-num="${pr.n}"
               style="left:${pr.x}%;top:${pr.y}%"
               onmouseenter="paramHover(${pr.n})" onmouseleave="paramUnhover()">
            ${pr.n}
          </div>` : '').join('') : ''}
      </div>` : '';
    ...
    const cardsHtml = `
      <div class="params-grid">
        ${params.map(pr => `
          <div class="param-card" data-num="${pr.n}"
               onmouseenter="paramHover(${pr.n})" onmouseleave="paramUnhover()">
            <div class="param-card-top">
              <span class="param-card-num">${pr.n}</span>
              <span class="param-card-name">${pr.name}</span>
            </div>
            <div class="param-card-range">${pr.range}</div>
            <div class="param-card-hover">
              <div class="param-card-default">
                <span class="param-label">По умолчанию:</span> ${pr.def}
              </div>
              <div class="param-card-tip">
                <span class="param-label">💡</span> ${pr.tip}
              </div>
            </div>
          </div>
        `).join('')}
  ...
  window.paramHover = function(num) {
    document.querySelectorAll(`.param-marker[data-num="${num}"]`)
      .forEach(el => el.classList.add('active'));
    document.querySelectorAll(`.param-card[data-num="${num}"]`)
      .forEach(el => el.classList.add('active'));
  };
  window.paramUnhover = function() {
    document.querySelectorAll('.param-marker.active, .param-card.active')
